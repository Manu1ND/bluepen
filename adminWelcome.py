...
import addJobs as addJobs
import adminJobs as adminJobs
...
def create_adminWelcome(rt, *args, **kwargs):
    '''Starting point when module is imported by another module.
       Correct form of call: 'create_adminWelcome(root, *args, **kwargs)' .'''
    global adminData
...
def searchJob(skill):
    ...
    with conn:
        ...
        for index, dat in enumerate(data):
            ...
            Button1 = tk.Button(
                Frame1, command=lambda dat=dat: deleteAdd(dat[0], skill))
...
def deleteAdd(jobID, skill):
    ...
    with conn:
        cur = conn.cursor()
        cur.execute('DELETE FROM jobs WHERE id LIKE ?', (jobID,))
        conn.commit()
    searchJob(skill)
...
class adminWelcome:
    def __init__(self, top=None):
        ...
        top.resizable(1, 1)
        top.title("Welcome admin")
        top.configure(background="#d9d9d9")

        self.Label1 = tk.Label(top)
        self.Label1.place(relx=0.28, rely=0.016, height=58, width=252)
        ...
        self.Button1 = tk.Button(
            top, command=lambda: addJobs.create_addJobs(root))
        self.Button1.place(relx=0.1, rely=0.151, height=63, width=236)
        ...
        self.Button1.configure(text='''Add Jobs''')

        self.Button2 = tk.Button(
            top, command=lambda: adminJobs.create_viewApp(root))
        self.Button2.place(relx=0.55, rely=0.151, height=63, width=236)
        ...
        self.Button2.configure(text='''View Applications''')
